...
where T: pgcodegen::db::SimpleClient<Row = postgres::Row, Err = postgres::Error>,
{
    type Err = <T as pgcodegen::db::SimpleClient>::Err;
    ...
    fn select<P>(mut client: T, pkey: &P) -> Result<Option<Self>, Self::Err>
    where P: postgres::types::ToSql,
          P: std::marker::Sync,
    {
        let selected = client.fetch("select id, source, key from eventrel where id = $1", &[pkey])?;
...
where T: pgcodegen::db::SimpleClient<Row = postgres::Row, Err = postgres::Error>,
{
    ...
    where P: postgres::types::ToSql,
          P: std::marker::Sync,
    {
        let selected = client.fetch("select participantid, instanceid, id, status from attendance where id = $1", &[pkey])?;
        let fields = match selected.get(0) {
        Some(fields) => fields,
        None => return Ok(None)
        };
...
where T: pgcodegen::db::SimpleClient<Row = postgres::Row, Err = postgres::Error>,
{
    ...
    where P: postgres::types::ToSql,
          P: std::marker::Sync,
    {
        let selected = client.fetch("select id, participant_id, first_name, last_name, email from participants where id = $1", &[pkey])?;
        let fields = match selected.get(0) {
        Some(fields) => fields,
        None => return Ok(None)
        };
...
where T: pgcodegen::db::SimpleClient<Row = postgres::Row, Err = postgres::Error>,
{
    ...
    where P: postgres::types::ToSql,
          P: std::marker::Sync,
    {
        let selected = client.fetch("select id, calendar_id, begin, end from calendarinstance where id = $1", &[pkey])?;
        let fields = match selected.get(0) {
        Some(fields) => fields,
        None => return Ok(None)
        };
...
where T: pgcodegen::db::SimpleClient<Row = postgres::Row, Err = postgres::Error>,
{
    ...
    where P: postgres::types::ToSql,
          P: std::marker::Sync,
    {
        let selected = client.fetch("select metadata_id, platform_type, date, id, kind, package_name, minute from appusageevents where id = $1", &[pkey])?;
        let fields = match selected.get(0) {
        Some(fields) => fields,
        None => return Ok(None)
        };
...
where T: pgcodegen::db::SimpleClient<Row = postgres::Row, Err = postgres::Error>,
{
    ...
    where P: postgres::types::ToSql,
          P: std::marker::Sync,
    {
        let selected = client.fetch("select repeats, id, organizer, description, repitition, days_of_week, image_url, color, image_description, title from events where id = $1", &[pkey])?;
        let fields = match selected.get(0) {
        Some(fields) => fields,
        None => return Ok(None)
        };
...
where T: pgcodegen::db::SimpleClient<Row = postgres::Row, Err = postgres::Error>,
{
    ...
    where P: postgres::types::ToSql,
          P: std::marker::Sync,
    {
        let selected = client.fetch("select inbound, platform_id, date_sent, id, date, thread_id, metadata_id, minute, body, address, recipient_id from smsdata where id = $1", &[pkey])?;
        let fields = match selected.get(0) {
        Some(fields) => fields,
        None => return Ok(None)
        };
...
where T: pgcodegen::db::SimpleClient<Row = postgres::Row, Err = postgres::Error>,
{
    ...
    where P: postgres::types::ToSql,
          P: std::marker::Sync,
    {
        let selected = client.fetch("select id, metadata_id, date, duration, recipiend_id from calldata where id = $1", &[pkey])?;
        let fields = match selected.get(0) {
        Some(fields) => fields,
        None => return Ok(None)
        };
...
where T: pgcodegen::db::SimpleClient<Row = postgres::Row, Err = postgres::Error>,
{
    ...
    where P: postgres::types::ToSql,
          P: std::marker::Sync,
    {
        let selected = client.fetch("select id, participant_id, device_id from metadata where id = $1", &[pkey])?;
        let fields = match selected.get(0) {
        Some(fields) => fields,
        None => return Ok(None)
        };
...
where T: pgcodegen::db::SimpleClient<Row = postgres::Row, Err = postgres::Error>,
{
    ...
    where P: postgres::types::ToSql,
          P: std::marker::Sync,
    {
        let selected = client.fetch("select id, event_id, begin, end, duration from eventinstances where id = $1", &[pkey])?;
        let fields = match selected.get(0) {
        Some(fields) => fields,
        None => return Ok(None)
        };
...
where T: pgcodegen::db::SimpleClient<Row = postgres::Row, Err = postgres::Error>,
{
    ...
    where P: postgres::types::ToSql,
          P: std::marker::Sync,
    {
        let selected = client.fetch("select metadata, id, selfattendeestatus, eventlocation, owneraccount, title, description from calendarentry where id = $1", &[pkey])?;
        let fields = match selected.get(0) {
        Some(fields) => fields,
        None => return Ok(None)
        };
...
where Self: Sized,
{
    ...
    fn select<P>(client: T, pkey: &P) -> Result<Option<Self>, Self::Err>
    where P: postgres::types::ToSql,
          P: std::marker::Sync,
    ;
